...
void FLIF_IMAGE::write_row_N(uint32_t row, const void* buffer, size_t buffer_size_bytes) {
	...
	if (nb_channels >= 4) {
		...
		for (size_t c = 0; c < (size_t)image.cols(); c++) {
			image.set(0, row, c, buffer_byte[c]);
		}
	}
}
...
void FLIF_IMAGE::read_row_N(uint32_t row, void* buffer, size_t buffer_size_bytes) {
	...
	if (nb_channels >= 4) {
		...
		// rgb
		FLIF_RGB* buffer_rgb = reinterpret_cast<FLIF_RGB*>(buffer);
		for (size_t c = 0; c < (size_t)image.cols(); c++) {
			buffer_rgb[c].r = ((image(0, row, c) >> rshift) * mult) & 0xFF;
			buffer_rgb[c].g = ((image(1, row, c) >> rshift) * mult) & 0xFF;
			buffer_rgb[c].b = ((image(2, row, c) >> rshift) * mult) & 0xFF;
		}
	}
	else if (nb_channels == 1) {
		// grayscale
		...
		for (size_t c = 0; c < (size_t)image.cols(); c++) {
			buffer_byte[c] = ((image(0, row, c) >> rshift) * mult) & 0xFF;
		}
	}
}
